  ...
  setInterval(function () {
    axios.get(API_LINK)
      .then(response => {
        ...
        let new_purchase_link_3080 = featuredProduct.retailers[0].purchaseLink;
        let new_purchase_link_3090 = productDetailList[0].retailers[0].purchaseLink;
        let new_purchase_link_3070 = productDetailList[1].retailers[0].purchaseLink;
        let new_purchase_link_3060 = productDetailList[2].retailers[0].purchaseLink;
        if (new_purchase_link_3060 != purchase_link_3060) {
          purchase_link_3060 = new_purchase_link_3060;
          bot.channels.get(Chanel_ID_3060).send('New purchase link: ' + purchase_link_3060);
        }

        if (new_purchase_link_3070 != purchase_link_3070) {
          purchase_link_3070 = new_purchase_link_3070;
          bot.channels.get(Chanel_ID_3070).send('New purchase link: ' + purchase_link_3070);
        }

        if (new_purchase_link_3080 != purchase_link_3080) {
          purchase_link_3080 = new_purchase_link_3080;
          bot.channels.get(Chanel_ID_3080).send('New purchase link: ' + purchase_link_3080);
        }

        if (new_purchase_link_3090 != purchase_link_3090) {
